...
// PaperSize string is mapped to a signintech/gopdf preset. Empty or
...
func New(ctx context.Context, c Config, fontsCache fonts.Cache) *Impl {
	...
	// both at page-open (signintech/gopdf gopdf.go:840-848), matching gofpdf's
	// scheduling — footer closures still need to absolutely position themselves
	// via SetY(-margin) if they want to sit at the bottom of the page.
	if c.HeaderFunc != nil {
	...
	// Preload the default text font before page 1. signintech/gopdf has no
...
// Impl wraps signintech/gopdf and mirrors graphics-state values that gopdf
...
type Impl struct {
	...
	// (baseline = cellTop + 0.5·h + 0.3·Size) on top of signintech/gopdf, which
...
func (f Impl) baselineShift(h float64) float64 {
	...
	// signintech/gopdf's SetFont matches the same way (style&^Underline).
...
func (f *Impl) AddFont(family string, styleStr string, data []byte) error {
	...
	// is already cached and signintech/gopdf would otherwise leak duplicate
	...
	// Parser errors are non-fatal: fall through and let signintech/gopdf
	// surface them on AddTTFFontDataWithOption; the draw path then falls
	// back to signintech's native baseline for that font.
